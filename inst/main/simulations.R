...
gaugePerf <- function(base_cnt, frac_dists=seq(0.1, 1, by=0.05), seed=1, 
                      nstart=10, multi=FALSE, pswitch=0.05){
  ...
  perfs <- lapply(frac_dists, function(frac_dist){
    ...
    set.seed(seed)
    draws <- genData(seed = sample(1:10000,1), N=N, cnts = cnts, switch_prob = pswitch)
...
#                   "low_cnts"=setNames(rep(bcnt[3], 3), c('LOH', 'H1', 'H2')),
#                   "vlow_cnts"=setNames(rep(bcnt[4], 3), c('LOH', 'H1', 'H2')))
# cnts_perf <- lapply(cnts_list, gaugePerf, frac_dists=fdists, nstart=20, 
#                     multi=FALSE, pswitch=0.0001)
# 
# cols <- brewer.pal(n = length(cnts_perf)+1, name = "PuBu")
# pdf("~/hmm_f1_3state.pdf")
# plot(0, type='n', xlim=c(0,1), ylim=c(0,1), xlab="fDist", ylab="F1", las=1)
